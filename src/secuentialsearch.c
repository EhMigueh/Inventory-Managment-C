...
int sequential_search(Inventory *inv, int id) {
    ...
    // Precalentar la caché (esta no mide es solo para cargar)
    ...
    // Realizar múltiples iteraciones para obtener una medición más precisa
    for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
        ...
        for (int i = 0; i < inv->count; i++) {
            // Forzar la comparación (previene optimizaciones agresivas del compilador)
            if (inv->products[i].id == id) {
                result = i;
                break;  
            }
...
int sequential_search_by_price_range(Inventory *inventory, double min_price, double max_price, Product **results, int max_results) {
    ...
    for (int i = 0; i < inventory->count; i++) {
        if (inventory->products[i].price >= min_price && 
            inventory->products[i].price <= max_price) {
            found_items++;
        }
    }
