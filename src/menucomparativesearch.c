...
void handle_comparative_search(Inventory *first_inv, Inventory *second_inv, Inventory *third_inv, Inventory *fourth_inv, Inventory *fifth_inv)
{
    ...
    if (search_option == 1 || search_option == 2)
    {
        ...
        const char *names_to_search[10] = {"Laptop", "Televiso", "Smartfon", "Auricular", "Tablet", "Impresora", "Monitor", "Teclado", "Mouse", "Cámara"};
        ...
        const char *plot_title = search_option == 1 ? "Comparación de Búsquedas por ID" : "Comparación de Búsquedas por Nombre";
        const char *plot_filename = search_option == 1 ? "Comparative search by id" : "Comparative search by name";

        // Asumiendo que tienes una función para graficar tres series de datos.
        plot_comparative_search_times(sizes, seq_times, bin_iter_times, bin_rec_times, 5, plot_title, plot_route, plot_filename);
            ...
            for (int rep = 0; rep < NUM_REPETITIONS; rep++)
                for (int j = 0; j < 10; j++)
                {
                    ...
                    if (rep == NUM_REPETITIONS - 1)
                    {
                        ...
                        for (int k = 0; k < dbs[i]->count; k++) // Simulación del conteo sin almacenar resultados.
                            if (dbs[i]->products[k].price >= price_ranges[j][0] && dbs[i]->products[k].price <= price_ranges[j][1])
                            {
                                found++;
                                if (found >= 5)
                                    break; // Simular el límite de resultados
                            }
                }
