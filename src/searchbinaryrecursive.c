...
int find_lower_bound_recursive(Inventory *inv, double min_price, int left, int right)
{
    ...
    if (inv->products[mid].price >= min_price) // Intenta encontrar otro producto válido más a la izquierda.
    {
...
int find_upper_bound_recursive(Inventory *inv, double max_price, int left, int right)
{
    ...
    if (inv->products[mid].price <= max_price) // Intenta encontrar otro producto válido más a la derecha.
    {
