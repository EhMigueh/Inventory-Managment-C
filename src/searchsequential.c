...
int sequential_search_by_id(Inventory *inv, int id)
{
    ...
    for (; i < inv->count - 3; i += 4)
    {
        if (inv->products[i].id == id)
            return i;
        if (inv->products[i + 1].id == id)
            return i + 1;
        if (inv->products[i + 2].id == id)
            return i + 2;
        if (inv->products[i + 3].id == id)
            return i + 3;
    }
...
int sequential_search_by_name(Inventory *inv, const char *name)
{
    ...
    for (int i = 0; i < inv->count; i++)
        if (strlen(inv->products[i].name) == name_len && inv->products[i].name[0] == name[0])
            if (strcmp(inv->products[i].name, name) == 0) // Si coinciden, hacemos la comparación completa.
                return i;
